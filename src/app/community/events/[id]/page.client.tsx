...
export default function EventDetailClientPage({ post: initialPost }: Props) {
  const router = useRouter();
  const queryClient = useQueryClient();
  const { data: post } = useQuery({
    queryKey: ["post", initialPost.id],
    queryFn: () =>
      fetch(`/api/community/events/${initialPost.id}`).then((res) =>
        res.json(),
      ),
  ...
  const cityLabel =
    CITY_OPTIONS.find((v) => v.value === post.city)?.label || post.city;
  ...
  const deleteMutation = useMutation({
    mutationFn: async () => {
      const res = await fetch(`/api/community/events/${initialPost.id}`, {
        method: "DELETE",
      });
          ...
          <header className={styles.header}>
            ...
            <div className={styles.metaSub}>
              <span>
                개최일 :{" "}
                {post.eventDate
                  ? format(new Date(post.eventDate), "yy.MM.dd")
                  : "-"}
              </span>

              <span>
                {cityLabel} {post.district}
              </span>
            ...
            <div className={styles.rightBtns}>
              {isOwner && (
                <Button
                  variant="edit"
                  href={`community/events/${initialPost.id}/edit`}
                >
                  수정
                </Button>
              )}
