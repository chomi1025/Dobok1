...
### 5. 메타 태그 및 SEO 디테일 최적화
각 페이지별로 title, description, og:image 등의 메타 태그를 동적으로 설정했습니다.
- 이를 통해:
  - SNS 공유 시 미리보기 품질 향상
  - 페이지별 SEO 강화
...
**[해결 방안]**
  1. 하이브리드 렌더링 도입 (ISR + CSR): 변경 빈도가 낮은 상품 데이터는 빌드 시점에 ISR로 미리 생성하고, 실시간 정보는 클라이언트에서 CSR로 보완하여 즉각적인 화면 렌더링 구현.
 ...
 4. Global Latency 최적화: Vercel 배포 리전을 **Seoul**로 변경하여 데이터 통신 경로를 단축
...
- [ ] **Oauth 로그인 구현:** 구글,카카오,네이버 등의 Oauth방식 로그인 구현
- [ ] **인증 보안 강화:** 서버 측 토큰 무효화 로직을 도입하여 강제 로그아웃 기능 구현
- [ ] **장바구니 고도화:** 비회원 상태에서 담은 상품을 로그인 시 회원 DB와 병합하는 로직
- [ ] **관리자 페이지 고도화:** 상품 수정/삭제 기능을 포함한 통합 관리 도구 구축 및 일일 매출, 주문 현황을 한눈에 파악할 수 있는 데이터 시각화 대시보드 구현
- [ ] **모바일 확장:** React Native를 활용한 크로스 플랫폼 앱 버전 개발 및 웹-앱 인증 동기화.
- [ ] **커뮤니티 및 메시징:** 구인구직 관련 1:1 쪽지 기능을 도입하고, 마이페이지 내 메시지 센터에서 통합 관리할 수 있는 시스템 구축
- [ ] **마케팅 기능 확장:** 사용자의 구매 유도를 위한 쿠폰 발행 시스템 및 등급별/기간 한정 할인 기능 추가.
- [ ] **실시간 소통**: Socket.io 혹은 WebRTC를 활용한 실시간 채팅 기능을 구현하여 구매자-판매자 간의 즉각적인 소통 지원.
